...
def get_next_click3D_torch_no_gt(prev_seg, img3D, threshold=170):
    ...
    pred_masks = prev_seg > mask_threshold # 获取当前分割预测掩码（二值化）
    likely_masks = img3D > threshold  # NOTE: Empirical threshold  获取可能是目标区域的掩码（基于图像强度阈值）
    fn_masks = torch.logical_and(likely_masks, torch.logical_not(pred_masks))  # 应该是目标区域（likely_masks为True）但当前没有被分割（pred_masks为False）的区域
    ...
    for i in range(prev_seg.shape[0]):  # , desc="generate points":
        ...
        point = None
        if len(fn_points) > 0:
            point = fn_points[np.random.randint(len(fn_points))]  # 随机选一个假阴性点
            is_positive = True # 标签为前景点
        # if no mask is given, random click a negative point
        if point is None:
            point = torch.Tensor(
                [np.random.randint(sz) for sz in fn_masks[i].size()]
            ).to(torch.int64)  # 随机选择一个点
            is_positive = False # 标签为背景点
